                                      ...
                                      const std::string & ee_frame)
{
  ...
  if(cfg("inherit_orientation", false))
  {
    R = ctl.robot().frame(ee_frame).position().rotation();
  }
...
struct CartesianMove : mc_control::fsm::State
{
  ...
  {
    duration_      = config("duration",      duration_);
    stiffness_     = config("stiffness",     stiffness_);
    weight_        = config("weight",        weight_);
    pos_threshold_ = config("pos_threshold", pos_threshold_);
    ori_threshold_ = config("ori_threshold", ori_threshold_);
    settle_timeout_ = config("settle_timeout", settle_timeout_);
    ee_frame_      = config("ee_frame",      ee_frame_);
    next_state_    = static_cast<std::string>(config("next", std::string("")));
    ...
    if(config.has("waypoints"))
    {
      ...
      {
        pos_waypoints_.emplace_back(static_cast<Eigen::Vector3d>(wp));
      }
    }
  }
...
struct JointMove : mc_control::fsm::State
{
  ...
  void configure(const mc_rtc::Configuration & config) override
  {
    ...
    stiffness_ = config("stiffness", stiffness_);
    weight_    = config("weight",    weight_);
    threshold_ = config("threshold", threshold_);
    next_state_ = static_cast<std::string>(config("next", std::string("")));

    // target: { joint_1: 0.0, joint_2: 0.25, ... }  OR  [v1, v2, ... v6]
    if(config.has("target"))
    {
      auto tcfg = config("target");
      if(tcfg.size() > 0) // it's an array
      {
        std::vector<double> vals = tcfg;
        for(size_t i = 0; i < vals.size(); ++i)
        {
          target_joints_["joint_" + std::to_string(i + 1)] = {vals[i]};
        }
      }
      else // assume it's a map
      {
        for(const auto & key : tcfg.keys())
        {
          target_joints_[key] = {static_cast<double>(tcfg(key))};
        }
      }
    }
  }
...
struct Gripper : mc_control::fsm::State
{
  ...
  void configure(const mc_rtc::Configuration & config) override
  {
    action_  = static_cast<std::string>(config("action",  action_));
    timeout_ = config("timeout", timeout_);
    next_state_ = static_cast<std::string>(config("next", std::string("")));
  }
